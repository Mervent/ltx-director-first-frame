...
app.registerExtension({ name: "DaSiWa.MiniMaxH3DirectorFirstFrame", nodeCreated(node) { if (node.comfyClass === "MiniMaxH3DirectorFirstFrame") install(node); }, loadedGraphNode(node) { if (node.comfyClass === "MiniMaxH3DirectorFirstFrame") { install(node); node.__dasiwaH3RestorePersistedState?.(); } } });
