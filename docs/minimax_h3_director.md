...
- **Discrete REF2VA reference-image sockets (`ref2va_image_1` … `ref2va_image_9`):** replaces the single `ref2va_images` IMAGE batch. Each reference is wired on its own socket at its own resolution/aspect ratio, matching how MiniMax H3 resizes every reference independently. A single shared batch tensor forced all references to identical dimensions (distorting differently-sized refs); the discrete sockets remove that limitation. A batch on any one socket is still split per frame, so same-size groups keep working.
- **Paired video-soundtrack socket (`ref2va_video_audio`):** a new AUDIO input that binds by index to the `ref2va_videos` frame batch, so a fully socket-driven REF2VA workflow can reference a video *with its own sound* (`<Video N>` + its `<Audio N>`) — the audio lands in `ref_video_audios` instead of as a standalone `<Audio N>`. Ignored when `ref2va_videos` is not connected. `ref2va_audios` still supplies independent audio references.
...
- **Optional REF2VA media sockets** (for fully graph-driven workflows, no timeline upload needed): `ref2va_image_1` … `ref2va_image_9` (nine independent IMAGE sockets, each wired at its own resolution/aspect ratio → the next `<Picture N>` in order; a batch on a single socket is still split per frame), `ref2va_videos` (IMAGE frame batch → one `<Video N>`; frames / 24 = duration, 2–15s), `ref2va_audios` (AUDIO → one standalone `<Audio N>`), and `ref2va_video_audio` (AUDIO paired by index to `ref2va_videos` as that video's soundtrack → `ref_video_audios`; ignored if no `ref2va_videos`). These append to any timeline refs before validation. Reference images use discrete sockets rather than one shared IMAGE batch because MiniMax H3 resizes each reference independently, so refs may differ in size — a single batch tensor would force them all to identical dimensions. Because `IMAGE` carries frames only, a socket video has no sound unless you also wire `ref2va_video_audio`.
- All media is managed inside the node UI (upload/paste/drop) or through the REF2VA media sockets above, but uploaded paths ultimately live in ComfyUI's `input/` folder.
