# Modified 2026-08-10 from LTX Director in WhatDreamsCost-ComfyUI
# (https://github.com/WhatDreamsCost/WhatDreamsCost-ComfyUI). GPL v3 — see LICENSE.
import logging
import asyncio
import json
import base64
...
def _resize_guide_tensor(tensor: torch.Tensor, custom_width: int, custom_height: int, resize_method: str, divisible_by: int) -> torch.Tensor:
    def snap(val, div):
        return max(div, (val // div) * div)

    src_h, src_w = tensor.shape[1], tensor.shape[2]
    if custom_width > 0 and custom_height > 0:
        return _resize_image(tensor, custom_width, custom_height, resize_method, divisible_by)
    if custom_width > 0:
        tgt_w = snap(custom_width, divisible_by)
        tgt_h = snap(int(src_h * tgt_w / src_w), divisible_by)
        return _resize_image(tensor, tgt_w, tgt_h, "stretch to fit", divisible_by)
    if custom_height > 0:
...
class LTXDirectorFirstFrame(io.ComfyNode):
    """WYSIWYG timeline variant — segments and lengths come from a visual editor in the node UI."""

    @classmethod
    def define_schema(cls):
        return io.Schema(
            node_id="LTXDirectorFirstFrame",
            display_name="LTX Director (First Frame Input)",
